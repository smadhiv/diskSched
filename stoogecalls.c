...
SYSCALL_DEFINE0(init_disksched){
  sum_of_services = 0;
  sum_of_waits = 0;
...
SYSCALL_DEFINE0(get_disksched){
  unsigned int avg_serv = (unsigned int)(sum_of_services  * 1000 / number_of_requests);
  unsigned int avg_wait = (unsigned int)(sum_of_waits * 1000 / number_of_requests);
  pr_alert(" %d\n%d\n%d", avg_serv, avg_wait, number_of_requests);
  return 0;
}
